...
<body onLoad = "myFunction()">
  ...
  <a class="navbar-brand" href="#"><b><?=$_SESSION['name']?></b></a>
      ...
      <div class="card">
        ...
        <H1>Registered users<h1><hr>
          <?php
            ...
            $users = "SELECT DISTINCT email FROM users WHERE email != '$email'";
           // $lastMessageStatus = mysqli_query($conn,$lastMessage) or die(mysqli_error($conn));
           $userStatus = mysqli_query($conn,$users) or die(mysqli_error($conn));
            if(mysqli_num_rows($userStatus) > 0) {
              while($lastMessageRow = mysqli_fetch_array($userStatus)) {
                $sent_by = $lastMessageRow['email'];
               ...
               // echo $lastMessageRow['email'];
          ?>
          <div class="card">
            <div class="card-body">
              <h6><strong><img src = "./dp/<?=$getSenderRow['dp']?>" alt = "dp" width = "40"/> <?=$getSenderRow['name'];?></strong><a href="./message.php?receiver=<?=$sent_by?>" class="btn btn-outline-primary" style = "float:right">Send message</a></h6>
            </div>
          </div><br/>
          <?php
            }
